...
#
# Two checkouts of verl - main before the merge (8e4a572) and main after it -
...
RUNNER = r'''
import json, sys
verl_path, family, model, arg_form, out = sys.argv[1:6]
...
def checkout(path: str, ref: str | None) -> str:
    if not os.path.isdir(os.path.join(path, "verl")):
        if ref:
            ...
            subprocess.run(f"rm -rf {path} && git clone -q --depth 1 {UPSTREAM_REPO} {path}", shell=True, check=True)
    return subprocess.check_output(["git", "-C", path, "rev-parse", "--short", "HEAD"], text=True).strip()


def run(verl_path: str, family: str, model: str, arg_form: str):
    out = tempfile.mktemp(suffix=".json")
    proc = subprocess.run(
        [sys.executable, "-c", RUNNER, verl_path, family, model, arg_form, out], capture_output=True, text=True
    )
...
def main() -> int:
    ...
    after = checkout("/tmp/verl_after", None)
    print(f"[fused] main before #7628 @{before} vs main after @{after}, tool_agent_loop-shaped trajectories, 24 cases per row")
    for family, candidates, arg_forms in FAMILY_MODELS:
        for model in candidates:
            for arg_form in arg_forms:
                old = run("/tmp/verl_before", family, model, arg_form)
                if isinstance(old, str):
                    print(f"[skip] {family} {model} args={arg_form}: {old}")
                    continue
                new = run("/tmp/verl_after", family, model, arg_form)
                if isinstance(new, str):
                    print(f"[skip] {family} {model} args={arg_form} (after): {new}")
                    continue
                same = differ = neither = ref_old = ref_new = with_ref = 0
                first_diff = None
                errors = set()
                for o, n in zip(old["cases"], new["cases"]):
                    if "ids" in o and "ids" in n:
                        ...
                        if "reference" in n:
                            with_ref += 1
                            ref_old += o["ids"] == n["reference"]
                            ref_new += n["ids"] == n["reference"]
                    else:
                        neither += 1
                        errors.add((o.get("error"), n.get("error")))
                ref_note = f"ref: before=={ref_old}/{with_ref} after=={ref_new}/{with_ref}" if with_ref else "ref: not prefix-stable"
                print(
                    f"[FUSED] {family:<9} {model:<30} args={arg_form:<4} thinking_switch={new['has_thinking_switch']} "
                    f"after==before {same}/24 differ={differ} not_rendered={neither} | {ref_note}"
                )
                if first_diff:
                    o, n = first_diff
                    print(f"[DIFF]  {family} {model} system={o['system_prompt']} prior={o['prior_turns']} calls={o['calls']} kwargs={o['kwargs']}")
                    print(f"[DIFF]    before: {o['text'][-170:]!r}")
                    print(f"[DIFF]    after : {n['text'][-170:]!r}")
                for err in sorted(errors, key=str)[:2]:
                    print(f"[ERR]   {family} {model} args={arg_form} before={err[0]} after={err[1]}")
